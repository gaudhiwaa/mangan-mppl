...
function AdminTransaction() {
  ...
  const getAPI = async (items) => {
    const trans = await axios.get("http://localhost:8080/transactions")
  ...
  const doneTrans = async(i) => {
    // console.log(APITransactions[i].id)
    try {
      await axios.patch(`http://localhost:8080/transactions/${APITransactions[i].id}`, {
        t_status: "Pesanan selesai"
      });
      console.log(APITransactions[i].id)
    } catch (error) {
      console.log(error); 
  ...
  return (
    ...
    >
      <AppBarTop text={"Transaksi"} addBasket navAddProd={"/adminproduct"} />
